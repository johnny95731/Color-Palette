...
<div class="favOffcanvas">
  <nav class="menuBar">
    <span v-for="(label, i) in pageLabels" :key="`page ${label}`"
    ...
    <TheIcon type="close" @click="$emit('fav-showing')"
    />
  </nav>
  <!-- Page content -->
  <ul class="pageContent">
    <template v-if="page === 0">
      <ColorBlock v-for="(hex) in favState.colors" :key="`favColor ${hex}`"
        :hex="hex"
      />
    </template>
    <template v-else-if="page === 1">
      <PaletteBlock v-for="(plt) in favState.plts" :key="`favPlt ${plt}`"
        :plt="plt" hex=""
      />
    </template>
  </ul>
...
const state = computed(() => {
  const isFavPlt = favState.plts.includes(plt.value);
  if (isFavPlt) {
    return {
      icon: 'unfavorPallete',
      text: 'Remove Pallete',
...
function favPltChanged() {
  favState.favPltsChanged(plt.value);
  setPage(1);
}
