...
import { getClosestNamed, getContrastAdjuster, getHarmonize, getSpaceInfos, hex2hsb, hex2rgb, hsb2hex, hueRotation, isValidHex, labFunc, labFuncInv, linearRgb2srgb, randRgbGen, rgb2gray, rgb2hex, sortingByLuminance, srgb2linearRgb, unzipCssNamed } from '@/utils/colors';
...
describe('linear rgb', () => {
  test('srgb2linearRgb', () => {
    const cases = [
      [[0,0,0], [0,0,0]],
      [[RGB_MAX,RGB_MAX,RGB_MAX], [RGB_MAX,RGB_MAX,RGB_MAX]],
    ] as const;
    for (const [args, expect_] of cases) {
      // @ts-expect-error
      const result = srgb2linearRgb(args);
      for (let i = 0; i < 3; i++) {
        expect(result[i]).toBeCloseTo(expect_[i]);
      }
    }
  });
  test('linearRgb2srgb', () => {
  ...
  test('stability', () => {
    ...
    for (let i = 0; i < testNum; i++) {
      const srgb = randRgbGen();
      const linearRgb = srgb2linearRgb(srgb);
      const srgb2 = linearRgb2srgb(linearRgb);
      const linearRgb2 = srgb2linearRgb(srgb2);
      const cases = [
        srgb,
        linearRgb,
        srgb2,
      ...
      for (let k = 0; k < 2; k++) {
        for (let i = 0; i < 3; i++) {
          expect(cases[k][i]).toBeCloseTo(cases[k+2][i]);
        }
      }
    }
  });
...
describe('lab helper', () => {
  test('stability', () => {
    ...
    for (let i = 0; i < testNum; i++) {
      ...
      const result = labFuncInv(labFunc(val));
